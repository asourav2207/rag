    ...
    for fname in os.listdir(excel_dir):
        if fname.endswith('.xlsx'):
            ...
            for idx, row in df.iterrows():
                ...
                def add_doc(text, column_name):
                    if text:
                        full_text = f"{column_name}: {text}\nField Name: {field_name}\nTable: {table_name}"
                        if full_text not in seen_texts:
                            docs.append(full_text)
                            metadata.append({'table': table_name, 'column': column_name, 'field': field_name})
                            seen_texts.add(full_text)

                add_doc(field_name, 'Field Name')
                add_doc(redshift_element, 'Redshift Element Name')
                add_doc(data_length, 'Data Length')
                add_doc(description, 'Description')
                add_doc(programmer_notes, 'Programmer Notes')
                add_doc(join_criteria, 'Joining Criteria SQL')
...
if st.button("Ask"):
    ...
    if user_query:
        with st.spinner("🧠 Thinking..."):
            answer, matches = answer_query(user_query, model, index, docs_list, metadata_list)
