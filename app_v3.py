...
def update_feedback(chat_id, fb):
    db = SessionLocal()
    entry = db.query(ChatHistory).get(chat_id)
    if entry:
        entry.feedback = fb
        db.commit()
    db.close()
...
entries = get_chat_history()
if st.session_state.get("user_has_asked", False):  # Show only after first question
    for entry in entries[::-1]:
        col1, col2 = st.columns([1, 1])
        with col1:
            if st.button("👍", key=f"up_{entry.id}"):
                update_feedback(entry.id, "up")
                st.experimental_rerun()
        with col2:
            if st.button("👎", key=f"down_{entry.id}"):
                update_feedback(entry.id, "down")
                st.experimental_rerun()
